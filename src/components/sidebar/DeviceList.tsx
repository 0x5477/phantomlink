  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto">
        {otherDevices.length === 0 && peers.length === 0 ? (
          <div className="p-4 text-center text-xs pl-text-dim">
            暂无联系人
            <br />
            <span className="opacity-60">点击 <Wifi size={11} className="inline" /> 扫描或 <UserPlus size={11} className="inline" /> 手动添加</span>
          </div>
        ) : (
          <>
            {peers.length > 0 && (
            ...
            {peers
              ...
              .map((peer) => (
                <button
                  key={`peer-${peer.device_id}`}
                  onClick={() => handleStartChat(peer.device_id)}
                ...
                >
                  ...
                  <UserPlus size={14} className="pl-text-cyan opacity-50" />
                </button>
              ))}
            ...
            {otherDevices.map((dev) => (
              <div
                key={dev.device_id}
                className="group w-full text-left px-3 py-3 flex items-center gap-3 hover:bg-white/3 transition-colors border-b border-white/3"
              >
                <button
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs pl-text-dim font-mono truncate">
                      {dev.fingerprint ? dev.fingerprint.substring(0, 23) + "..." : dev.device_id.substring(0, 12)}
                    </p>
                  </div>
                </button>
                <button
                  onClick={() => setDeleteConfirm(dev.device_id)}
                  className="pl-btn-ghost rounded p-1.5 opacity-0 group-hover:opacity-100 transition-opacity"
                  title="删除联系人"
                >
      ...
      {deleteConfirm && (
        ...
        >
          <div
            className="pl-glass-strong pl-glow-cyan rounded-2xl p-6 w-[320px]"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-sm text-center mb-4">确认删除该联系人？</h3>
            <p className="text-xs pl-text-dim text-center mb-5">
              将同时删除与此联系人的所有聊天记录
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => setDeleteConfirm(null)}
                className="pl-btn-ghost flex-1 py-2 rounded-lg text-sm"
              >
                取消
              </button>
              <button
                onClick={() => handleDelete(deleteConfirm)}
                className="flex-1 py-2 rounded-lg text-sm bg-red-500/20 border border-red-400/40 text-red-400"
              >
                删除
              </button>
            </div>
...
function AddFriendModal({
  ...
  return (
    ...
    >
      <div
        className="pl-glass-strong pl-glow-cyan rounded-2xl p-6 w-[380px]"
        onClick={(e) => e.stopPropagation()}
      >
        <h3 className="text-sm pl-text-cyan mb-4 flex items-center gap-2">
          <UserPlus size={16} />
          添加好友
        </h3>
        ...
        <div className="flex gap-2 mt-5">
          ...
          <button
            ...
            disabled={loading}
            className="pl-btn-primary flex-1 py-2 rounded-lg text-sm flex items-center justify-center gap-2"
          >
