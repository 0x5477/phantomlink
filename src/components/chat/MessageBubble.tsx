...
import { Flame, Lock, Check, CheckCheck, Clock, AlertCircle } from "lucide-react";
...
export default function MessageBubble({
 ...
 const settings = useStore((s) => s.settings);
  ...
  const StatusIcon = () => {
    ...
    switch (msg.status) {
      case "pending":
        return <Clock size={12} className={cls + " pl-text-dim"} />;
      case "sent":
        return <Check size={14} className={cls + " pl-text-dim"} />;
      case "delivered":
        return <CheckCheck size={14} className={cls + " pl-text-dim"} />;
      case "read":
        return <CheckCheck size={14} className={cls + " pl-text-cyan"} />;
      case "failed":
        return <AlertCircle size={14} className={cls + " text-red-400"} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div
      className={`flex ${isSelf ? "justify-end" : "justify-start"} pl-msg-${isSelf ? "sent" : "received"}`}
    >
