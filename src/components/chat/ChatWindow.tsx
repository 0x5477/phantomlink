    ...
    <div className="flex flex-col h-full">
      <div className="pl-glass border-b border-white/5 px-5 py-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          <h2 className="text-sm font-medium">{conv?.display_name || "Unknown"}</h2>
        ...
        {peer && (
          <button onClick={startVoiceCall} className="pl-btn-ghost rounded-lg p-2" title="语音通话">
            <Phone size={16} className="pl-text-cyan" />
          </button>
        )}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4 space-y-2">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center pl-text-dim">
            ...
            <p className="text-xs mt-1 opacity-60">所有消息均经过 AES-256-GCM 端到端加密</p>
          </div>
        ) : (
          messages.map((msg) => (
            <MessageBubble
