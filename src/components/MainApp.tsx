...
export default function MainApp() {
  ...
  const setupAutoLock = useStore((s) => s.setupAutoLock);
  ...
  useEffect(() => {
    ...
    // Reset auto-lock on any user interaction
    const resetLock = () => setupAutoLock();
  ...
  }, [setupEventListeners, setupAutoLock]);
...
function EmptyState({ deviceName }: { deviceName: string | null }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center pl-text-dim">
      ...
      <p className="text-sm">
        {deviceName ? `欢迎回来，${deviceName}` : "选择一个会话或联系人开始聊天"}
      </p>
