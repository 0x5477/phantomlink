...
/// Manually add a friend by IP address. Creates a device entry and attempts connection.
#[tauri::command]
fn add_device(
    ip: String,
    port: Option<u16>,
    ...
    if connect_result.is_ok() {
        // Send Pair frame with our identity
        ...
        log::warn!(
            "Could not connect to {ip}:{port}, device saved for later retry"
        );
    }
...
fn delete_device(
    ...
    let db_key = state.app.get_db_key()?;
    // Also clean up related conversations
...
/// Construct and send a chat message frame to a peer.
#[tauri::command]
fn send_message_frame(
    ...
    let frame = Frame::Message {
        ...
        nonce: nonce_b64,
        flags: if burn_after_read {
            vec!["burn_after_read".to_string()]
        } else {
            vec![]
        },
        reply_to: None,
...
/// Send a file (metadata + chunked data) to a peer.
#[tauri::command]
fn send_file_frame(
    peer_device_id: String,
    message_id: String,
    ...
    let file_rec = state
        .db
        .get_file_record(&db_key, &file_id)?
        .ok_or("file not found")?;

    let file_data = state.file_store.load(&file_key, &file_rec.stored_name)?;

    let chunk_size: usize = 262144; // 256 KB
    let total_chunks = ((file_data.len() + chunk_size - 1) / chunk_size) as i64;

    let meta_frame = Frame::FileMeta {
        file_id: file_id.clone(),
        message_id: message_id.clone(),
        name: file_rec.original_name.clone(),
        size: file_rec.size,
        mime: file_rec.mime_type.clone(),
        sha256: file_rec.sha256.clone(),
        total_chunks,
        chunk_size: chunk_size as i64,
        is_image: file_rec.is_image,
        thumbnail_b64: None,
        width: file_rec.width,
        height: file_rec.height,
    };
    state
        .network
        .send_to_peer(&peer_device_id, protocol::encode_frame(&meta_frame))?;
    ...
    for i in 0..total_chunks {
        ...
        let chunk_frame = Frame::FileChunk {
            file_id: file_id.clone(),
            chunk_index: i,
            encrypted_data: chunk_b64,
            is_last: i == total_chunks - 1,
        };
        state
            .network
            .send_to_peer(&peer_device_id, protocol::encode_frame(&chunk_frame))?;
    }
...
// ---- Frame consumer (processes incoming P2P frames) ----
    ...
    rt.spawn(async move {
        ...
        while let Some((source, frame_bytes)) = frame_rx.recv().await {
            ...
            let frame = match Frame::from_json(&frame_str) {
                Ok(f) => f,
                Err(e) => {
                    log::debug!("Failed to parse frame from {source}: {e}");
                    continue;
                }
            };
            ...
            match frame {
                Frame::Pair {
                    ...
                    let db_key = match state.app.get_db_key() {
                        Ok(k) => k,
                        Err(_) => continue,
                    };
                    let pubkey = base64::engine::general_purpose::STANDARD
                        ...
                        .unwrap_or_default();
                    let fingerprint = if !pubkey.is_empty() {
                        crypto::fingerprint_hex(&pubkey)
                    } else {
                        String::new()
                    };
                    ...
                    // Respond with our own Pair frame so the initiator learns our identity
                    let our_id = state.db.meta_string("device_id").unwrap_or_default();
                    if our_id != sender_id {
                        let our_pubkey = state
                            ...
                            .unwrap_or_default();
                        let our_pubkey_b64 =
                            base64::engine::general_purpose::STANDARD.encode(&our_pubkey);
                        let our_name = state
                            .db
                            .get_device_by_id(&db_key, &our_id)
                            .ok()
                            .flatten()
                        ...
                        let _ = state
                            .network
                            .send_to_peer(&sender_id, protocol::encode_frame(&resp));
                    }
                ...
                Frame::Message {
                    ...
                    let content =
                        match base64::engine::general_purpose::STANDARD.decode(&encrypted_payload) {
                    ...
                    let convs = match state.db.get_conversations(&db_key) {
                        Ok(c) => c,
                        Err(_) => continue,
                    };
                    let conv = if let Some(existing) = convs
                        .iter()
                        .find(|c| c.peer_device_id.as_deref() == Some(&sender_id))
                    {
                        existing.clone()
                    } else {
                        let peer = state.db.get_device_by_id(&db_key, &sender_id)
                            .ok()
                            .flatten();
                        let display_name = peer
                            .map(|p| p.display_name)
                            .unwrap_or_else(|| {
                                sender_id[..8.min(sender_id.len())].to_string()
                            });
                        let new_conv = Conversation {
                            conv_id: uuid::Uuid::new_v4().to_string(),
                            conv_type: "private".to_string(),
                            peer_device_id: Some(sender_id.clone()),
                            group_id: None,
                            display_name,
                            last_message_at: 0,
                            unread_count: 0,
                    ...
                    let _ = state
                        ...
                        .send_to_peer(
                            &sender_id,
                            protocol::encode_frame(&Frame::Ack {
                                message_id: message_id.clone(),
                ...
                Frame::FileChunk {
                    ...
                    let complete = {
                        ...
                        if let Some(ifile) = files.get_mut(&file_id) {
                            if let Ok(data) =
                                base64::engine::general_purpose::STANDARD.decode(&encrypted_data)
                            {
                    ...
                    if complete {
                        ...
                        if let Some(ifile) = ifile {
                            ...
                            let file_key = match state.app.get_file_key() {
                                Ok(k) => k,
                                Err(_) => continue,
                            };
                            ...
                            let stored_name = match state
                                .file_store
                                .store(&file_key, &ifile.received_data)
                            {
                                Ok(n) => n,
                                Err(e) => {
                                    log::error!("store incoming file: {e}");
                                    continue;
                                }
                            };
                            ...
                            let record = FileRecord {
                                ...
                                message_id: ifile.message_id.clone(),
                                original_name: ifile.name.clone(),
                                stored_name,
                                mime_type: ifile.mime.clone(),
                                size: ifile.size,
                                sha256: ifile.sha256.clone(),
                                is_image: ifile.is_image,
                                width: ifile.width,
                                height: ifile.height,
                                created_at: now_ms(),
                            };
                            ...
                            // Link file to the message that preceded the file transfer
                            let _ =
                                state
                                    .db
                                    .update_message_file_id(&ifile.message_id, &new_file_id);

                            let _ = app_handle.emit(
                                "file-received",
                                serde_json::json!({
                                    "message_id": ifile.message_id,
                                    "file_id": new_file_id,
                                }),
                            );
                        }
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            {
                if let Some(window) = app.get_webview_window("main") {
                    // Use Mica on Win11; fall back silently on older Windows
