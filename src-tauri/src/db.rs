...
impl Database {
    ...
    pub fn get_devices(&self, key: &Zeroizing<[u8; 32]>) -> Result<Vec<Device>, String> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT device_id, display_name, public_key, fingerprint, trusted, last_seen FROM devices")
            .map_err(|e| format!("db prepare: {e}"))?;
        let rows = stmt
            .query_map([], |row| {
                let name_enc: String = row.get(1)?;
                let pubkey: Vec<u8> = row.get(2)?;
                Ok((row.get::<_, String>(0)?, name_enc, pubkey, row.get::<_, String>(3)?, row.get::<_, i32>(4)?, row.get::<_, i64>(5)?))
            })
            .map_err(|e| format!("db query: {e}"))?;

        let mut devices = Vec::new();
        for r in rows {
            let (device_id, name_enc, pubkey, fingerprint, trusted, last_seen) =
